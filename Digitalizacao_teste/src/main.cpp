...
void loop() 
{
    ...
    /*status_atual = digitalRead(PIN_STATUS); //Ler pino de status
    state_atual_contangem = digitalRead(PIN_CONT); //Ler pino contador
    if((status_atual != status_ultimo) || ((state_atual_contangem != state_ultimo_contangem) && state_atual_contangem)){
      cont += 1;
      sprintf(mydata,"status: %s;contagem: %d", status_atual == false ? "0" : "1", cont); //Mescla informacoes das variáveis em 'mydata'
      do_send(&sendjob); //Envia mensagem: mydata
      delay(50);
    }
